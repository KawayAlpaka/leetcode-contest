...
var loudAndRich = function(richer, quiet) {
    var moreRichers = [];
    richer.forEach(function(rich){
        var her = rich[0];
        var ler = rich[1];
        if(moreRichers[ler] ){
            moreRichers[ler].push(her);
        } else{
            moreRichers[ler] = [];
            moreRichers[ler].push(her);
        }
    ...
    //去除重复
    var len1 = moreRichers.length;
    for(let i=0;i<len1;i++){
        if(moreRichers[i]){
            moreRichers[i].forEach(function(richer1){
                if(moreRichers[richer1]){
                    moreRichers[richer1].forEach(function(richer2){
                        var index = moreRichers[i].indexOf(richer2);
                        if(index >= 0){
                            moreRichers[i].splice(index,1);
                        }
                    })
                }
            });
    ...
    // console.log(moreRichers);
    var getRicher1 = function(n1){
        var richers = [];
        var getRicher2 = function(n){

            if(!moreRichers[n]){
                return [];
            }
            richers.push(...moreRichers[n]);
            var len = moreRichers[n].length;
            moreRichers[n].forEach(function(n){
                getRicher2(n);
            });
        };
        getRicher2(n1);
    ...
    var answer = [];
    for(let i=0;i<N;i++){
        var richers = getRicher1(i);
        var min = quiet[i];
        var quieter = i;
        richers.forEach(function(richer){
            if(quiet[richer] < min){
                min = quiet[richer];
                quieter = richer;
            }
    ...
    //     var her = rich[0];
    //     var ler = rich[1]; 
    //     // console.log("her=",her,"  ler",ler);
    //     var hIndex = richlist.indexOf(her);
    //     var lIndex = richlist.indexOf(ler);
    ...
    //     }
    //     if(hIndex >= 0 && lIndex<0){
    //         richlist.splice(hIndex,0,ler);
    //     }
    //     if(hIndex < 0 && lIndex>=0){
    //         richlist.splice(lIndex+1,0,her);
    //     }
    //     if(hIndex < 0 && lIndex<0){
    //         richlist.push(ler);
    //         richlist.push(her);
